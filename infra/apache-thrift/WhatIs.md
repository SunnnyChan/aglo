...
```md
	Schema处理
		Thrift依赖IDL-->代码的生成，静态的。走代码生成，编译载入的流程。
		Avro可以生成代码，后编译执行，但是还必须依赖IDL（meta元数据描述）
			也可以走动态解释执行IDL
	序列化的方式
		thrift提供多种序列化实现
			TCompactProtocol，TBinaryProtocol
			每个Field前面都是带Tag的，这个Tag用于标识这个域的类型和顺序ID（IDL中定义，用于Versioning）
			在同一批数据里面，这些Tag的信息是完全相同的，当数据条数大的时候这显然就浪费了。
		Avro，格式包括--》文件头中有schema+数据records(自描述)
			 只对感兴趣的部分反序列化
			schema允许定义数据的排序order
			采用block链表结构，突破了用单一整型表示大小的限制
				比如Array或Map由一系列Block组成，每个Block包含计数器和对应的元    素，计数器为0标识结束。
	RPC的服务
		Avro提供了
